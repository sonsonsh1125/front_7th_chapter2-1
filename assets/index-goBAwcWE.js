...
`,l=({title:e,image:t,productId:n,lprice:r,brand:i})=>`
    <div
  ...
  `,u=({loading:e=!1,products:t=[]}={})=>`
    ...
    ${c}`:`<div class="mb-4 text-sm text-gray-600">
  ...
  `,f=({filters:e,pagination:t,products:n,loading:r})=>d({children:`
      ${o({filters:e,pagination:t})}
      ${u({loading:r,products:n})}
    `});async function p(e={}){let{limit:t=20,search:n=``,category1:r=``,category2:i=``,sort:a=`price_asc`}=e,o=e.current??e.page??1,s=new URLSearchParams({page:o.toString(),limit:t.toString(),...n&&{search:n},...r&&{category1:r},...i&&{category2:i},sort:a}),c=await fetch(`/api/products?${s}`);return await c.json()}async function m(e){let t=await fetch(`/api/products/${e}`);return await t.json()}const h=({loading:e,product:t})=>d({children:e?` <div class="py-20 bg-gray-50 flex items-center justify-center">
        ...
        `}),g=()=>r(async()=>{let{worker:e}=await import(`./browser-CcyfQrG1.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`,serviceWorker:{url:`/front_7th_chapter2-1/mockServiceWorker.js`}}));async function _(){let e=document.getElementById(`root`);if(location.pathname===`/`){e.innerHTML=f({loading:!0});let t=await p();e.innerHTML=f({...t,loading:!1}),document.addEventListener(`click`,e=>{if(e.target.closest(`.product-card`)){let t=e.target.closest(`.product-card`).dataset.productId;history.pushState({},``,`/product/${t}`),_()}})}else{e.innerHTML=h({loading:!0});let t=location.pathname.split(`/`).pop(),n=await m(t);e.innerHTML=h({product:n,loading:!1})}}window.addEventListener(`popstate`,()=>{_()});function v(){_()}g().then(v);